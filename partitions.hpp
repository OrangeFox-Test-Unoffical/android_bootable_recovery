...
*/
enum PartitionManagerOp {                                                    // PartitionManager Restore Mode for Raw_Read_Write()
	PM_BACKUP = 0,
	PM_RESTORE = 1,
};
...
struct PartitionSettings {                                                        // Settings for backup session
	...
	enum PartitionManagerOp PM_Method;                                       // Current operation of backup or restore
};
...
public:
	...
	// TWRP16 高性能路径:Defer_Folder_Size 表示 /data/media 的目录统计交给后台线程(Update_Data_Size_Async)
	bool Update_Size(bool Display_Error, bool Defer_Folder_Size = false);     // Updates size information
	void Update_Data_Size_Async();                                            // 启动后台 data/media 占用扫描
	void Apply_Async_Data_Size();                                             // 在调用线程套用后台扫描结果
	static bool Data_Is_Locked();                                             // user 0 的 CE 存储是否仍锁定
...
private:
	...
	bool Backup_Size_Provisional;                                             // Backup_Size 目前是 statfs 值,目录统计仍在后台进行
...
public:
	...
	// TWRP16: Defer_Data_Size 让 /data 的 media 统计走后台,Display_Error 控制早期挂载错误是否上报
	void Update_System_Details(bool Defer_Data_Size = false, bool Display_Error = true); // Updates fstab, file systems, sizes, etc.
	void Process_Async_Data_Size();                                           // 套用后台算出的 data 大小
