...
func copyThemeResources(ctx android.BaseContext, dirs []string, files []string) {
	// 保持与上游 OrangeFox R11.3 一致:用环境变量 OUT(由 soong_ui 传给 soong_build)。
	// 注意不能用 ctx.Config().OutDir():本树实测它返回空串(""),
	// 于是 outDir 只剩相对路径 "target/product/myron",主题会被写到 cwd 下的错误位置。
...
func libGuiDefaults(ctx android.LoadHookContext) {
	...
	// FOX_*/OF_* 宏:走 fox_common.go(与 recovery 插件同一份实现,取自 ../fox_common.go)
	f := &props{}
	f.Cflags = foxGlobalFlags(ctx)
