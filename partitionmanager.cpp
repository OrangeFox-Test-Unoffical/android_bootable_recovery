	...
	if (Decrypt_Data && Decrypt_Data->Is_Encrypted && !Decrypt_Data->Is_Decrypted) {
		...
		if (!Decrypt_Data->Key_Directory.empty()) {
			...
			if (android::vold::fscrypt_mount_metadata_encrypted(
                    ...
                    Decrypt_Data->Current_File_System, false, user_devices, device_aliased, 0,
...
void TWPartitionManager::Output_Partition(TWPartition* Part) {
	// Size / Used / Free / Backup_Size 都是字节(见 partitions.hpp),统一交给 UnitConversion 自适应显示
...
// DJ9 20/08/2018 }
	...
	// 保持字节:下面用 UnitConversion::FormatBytes(actual_backup_size) 显示(上游同样去掉了这里的 /= 1MiB)
...
void TWPartitionManager::Update_System_Details(bool Defer_Data_Size, bool Display_Error) {
	int data_size = 0;   // MB(本树沿用 OF 约定)
	TWPartition *Deferred = nullptr;

	// OrangeFox: 跑 OTA survival 备份时不刷这条提示
	...
	for (TWPartition *partition: Partitions) {
		...
		// OrangeFox: 默认不上报 "无害的挂载问题";定义了 OF_REPORT_HARMLESS_MOUNT_ISSUES 则强制上报
#ifdef OF_REPORT_HARMLESS_MOUNT_ISSUES
		bool reporter = true;
#else
		bool reporter = Display_Error;
#endif
		// TWRP16: 只有 /data 值得延迟(它才喂 TW_BACKUP_DATA_SIZE)
		bool defer = Defer_Data_Size && partition->Has_Data_Media && partition->Mount_Point == "/data";
		partition->Update_Size(reporter, defer);
		if (defer)
			Deferred = partition;
		if (partition->Can_Be_Mounted) {
			// 注意:TW_BACKUP_*_SIZE 系列沿用"MB 整数"约定 —— 主题里会与 %tw_min_system% 做数值比较
			// (如 <condition var1="tw_backup_system_size" op=">=" var2="%tw_min_system%"/>),不要改成格式化字符串。
			if (partition->Mount_Point == Get_Android_Root_Path()) {
				DataManager::SetValue(TW_BACKUP_SYSTEM_SIZE, (int)(partition->Backup_Size / 1048576LLU));
			} else if (partition->Mount_Point == "/data" || partition->Mount_Point == "/datadata") {
				data_size += (partition->Backup_Size / 1048576LLU);
			} else if (partition->Mount_Point == "/cache") {
				DataManager::SetValue(TW_BACKUP_CACHE_SIZE, (int)(partition->Backup_Size / 1048576LLU));
			} else if (partition->Mount_Point == "/sd-ext") {
				DataManager::SetValue(TW_BACKUP_SDEXT_SIZE, (int)(partition->Backup_Size / 1048576LLU));
				...
				DataManager::SetValue(TW_BACKUP_ANDSEC_SIZE, (int)(partition->Backup_Size / 1048576LLU));
				if (partition->Backup_Size == 0) {
				...
				DataManager::SetValue(TW_BACKUP_BOOT_SIZE, (int)(partition->Backup_Size / 1048576LLU));
				if (partition->Backup_Size == 0) {
			...
			if (partition->Mount_Point == "/boot") {
				...
				DataManager::SetValue(TW_BACKUP_RECOVERY_SIZE, (int)(partition->Backup_Size / 1048576LLU));
				if (partition->Backup_Size == 0) {
	...
	if (FreeStorage) {
		...
		if (!FreeStorage->Mount(false)) {
			// OrangeFox: /usb_otg 挂不上是常态,不要弹警告
	...
	if (!Write_Fstab())
		LOGERR("Error creating fstab\n");
	// TWRP16: /data 处于锁定状态时先不做目录统计 —— 锁着的 data 里没有可备份的内容
	if (Deferred && !TWPartition::Data_Is_Locked())
		Deferred->Update_Data_Size_Async();
	return;
}

/* TWRP16: 套用后台算出的 /data 大小(在需要该数值的线程上调用) */
...
int TWPartitionManager::Partition_SDCard(void)
{
  ...
  // 这里的 total_size/fat_size/ext/swap 按 MB 计(tw_sdext_size/tw_swap_size 的 MB 约定,且要拼进 sgdisk 参数),
  // 所以保留 %iMB,不套 UnitConversion。
  LOGINFO
	  ...
	  if ((*iter)->Is_Storage)
	    {
	      ...
	      part.Display_Name += UnitConversion::FormatBytes((*iter)->Free);;
	  ...
	  if (((*iter)->Can_Be_Backed_Up || (((*iter)->Can_Be_Adv_Backed_Up) && (DataManager::GetIntValue("fox_adv_backup") != 0 ))) && !(*iter)->Is_SubPartition
	      ...
	      if ((*iter)->Has_SubPartition)
		{
		  std::vector < TWPartition * >::iterator subpart;
				...
				// Backup_Size 是字节,统一用 UnitConversion 显示(自带单位,不再拼 {@mbyte})
				part.Display_Name = (*iter)->Backup_Display_Name + " (";
				part.Display_Name += UnitConversion::FormatBytes(Backup_Size);
				part.Display_Name += ")";
  ...
  for (iter = Partitions.begin(); iter != Partitions.end(); iter++)
      ...
      if ((*iter)->Can_Be_Mounted)
	{
	  if ((*iter)->Mount_Point == Get_Android_Root_Path())
	    {
	      // 同 Update_System_Details:这些值保持 MB 整数(主题做数值比较)
  ...
  if (FreeStorage != NULL)
      ...
      else
	{
	  DataManager::SetValue(TW_STORAGE_FREE_SIZE,
				UnitConversion::FormatBytes(FreeStorage->Free));
	}
    }
  else
    {
