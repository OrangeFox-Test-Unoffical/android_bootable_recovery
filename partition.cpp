...
bool TWPartition::Process_Fstab_Line(const char *fstab_line, bool Display_Error, std::map<string, Flags_Map> *twrp_flags) {
	...
	if (Mount_Point == "/mnt/vendor/persist" && Can_Be_Mounted) {
		...
		if (mounted || Mount(false)) {
			...
			#endif
			TWFunc::Fixup_Time_On_Boot("/mnt/vendor/persist/time/");
			if (!mounted)
				UnMount(false);
		}
	}
...
bool TWPartition::Wipe(string New_File_System) {
	...
	if (Has_Data_Media && Current_File_System == New_File_System) {
		...
		if (check || Use_Rm_Rf)
			...
			wiped = Wipe_NTFS();
		// mtdutils(yaffs2/MTD)已作为遗留代码删除 —— 这里不再有 Wipe_MTD() 分支
...
bool TWPartition::Wipe_EXTFS(string File_System) {
	...
	if (TWFunc::Path_Exists("/system/bin/e2fsdroid")) {
		...
		if (!selinux_handle || selabel_lookup(selinux_handle, &secontext, File_Contexts_Entry.c_str(), S_IFDIR) < 0) {
			...
			if (Mount_Point == "/mnt/vendor/persist") {
				Mount(true);
				TWFunc::removeDir("/mnt/vendor/persist/lost+found", false);
				UnMount(true);
			}
...
/* ===== OrangeFox 移植:TWRP16 高性能 data-size 机制(原样来自 TWRP16 3.7.1_16) ===== */
...
bool TWPartition::Update_Size(bool Display_Error, bool Defer_Folder_Size) {
	...
	ro = Mount_Read_Only;
	Mount_Read_Only = true;
	// 本树 libbase 没有 make_scope_guard,直接用 ScopeGuard(退出作用域时恢复 Mount_Read_Only)
	android::base::ScopeGuard restore_ro([&]() { Mount_Read_Only = ro; });
